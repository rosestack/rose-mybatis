...
import io.github.rosestack.encrypt.annotation.EncryptField;
            ...
            if (encryptField != null) {
                try {
                    ...
                    if (value instanceof String) {
                        ...
                        if (metrics != null) {
                            var sample = metrics.startEncrypt();
                            try {
                                encryptedText = fieldEncryptor.encrypt(plainText, encryptField.value());
                            } finally {
                                metrics.recordEncrypt(sample);
                            }
                        } else {
                            encryptedText = fieldEncryptor.encrypt(plainText, encryptField.value());
                        }
                    ...
                    if (metrics != null) metrics.incrementEncryptError();
            ...
            if (encryptField != null) {
                try {
                    ...
                    if (value instanceof String) {
                        ...
                        if (metrics != null) {
                            var sample = metrics.startDecrypt();
                            try {
                                decryptedText = fieldEncryptor.decrypt(cipherText, encryptField.value());
                            } finally {
                                metrics.recordDecrypt(sample);
                            }
                        } else {
                            decryptedText = fieldEncryptor.decrypt(cipherText, encryptField.value());
                        }
                    ...
                    if (metrics != null) metrics.incrementDecryptError();
